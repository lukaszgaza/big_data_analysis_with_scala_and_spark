...
object WikipediaRanking {
  ...
  val conf: SparkConf = new SparkConf().setMaster("local").setAppName("Programming Languages Wikipedia Ranking")
  ...
  def rankLangsReduceByKey(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] =
    rdd
      .flatMap(article => langs.filter(lang => article.mentionsLanguage(lang)).map(lang => (lang, 1)))
